...
m.prototype.create = function() {
  var a = this;
  chrome.notifications.create(this.a, {type:"progress", title:"Đang đăng lên server của thầy", message:l.e("screenshot_plugin_uploading_window_capt"), iconUrl:caidat.pluginfolder+"img/icon128.png", buttons:[{title:l.e("screenshot_plugin_cancel")}], progress:0}, function() {
    a.i = "uploading";
  });
};
...
function F(a, b) {
	//Hàm này là hàm trả về đường dẫn từ http://i.imgur.com/
  b.V === a.c.n.D ? (a.H = b.A.img_url, a.j.cmdAfterUpload === J || a.j.cmdAfterUpload == "bsquochoai_save" ? chrome.runtime.sendMessage({name:M, id:a.a}) : (b.A.dpr = a.j.dpr, I.d("upload", "prntscr", "attempt"), z.save(b.A, function(b) {
...
(function() {
  ...
  function c(a) {
    d(a, function() {
      if (u[a] && u[a].cmdAfterUpload) { //@@Hàm xử lý nút sau khi đã đăng ảnh lên server -> trả về link ảnh @@Xử lý save ảnh và copy đường dẫn vào clipboard
        var b = R(k, a);
        switch(u[a].cmdAfterUpload) {
            ...
            chrome.tabs.create({url:"http://www.google.com/searchbyimage?image_url=" + b});
				chrome.storage.local.get("bs_pageshot", function(re){
					if(typeof re.bs_pageshot == "undefined"){
						re.bs_pageshot = {links: []}
					}
					time = new Date()
					re.bs_pageshot.links.push({img: b, time: time.customFormat( "#DD#/#MM#/#YYYY# #hh#:#mm#:#ss#")})
          ...
          case "bsquochoai_save":
			  ...
			  document.execCommand('Copy');
			  input.remove();
			  chrome.notifications.clear("bsquochoai_dacopyxong",function(){});
			  chrome.notifications.create("bsquochoai_dacopyxong", {
					type:"basic",
					title:"Đã upload và copy link!",
					iconUrl:caidat.pluginfolder+"img/icon78.png",
					message:"Giờ em đã có thể ấn Ctrl+V để đưa link vào bất cứ nơi nào rồi.\n"+b
				},function(){})
				motabmoi = localStorage.bs_pageshot_motabmoi_saukhiupload || "true"
				console.log(motabmoi)
				if( motabmoi == "true"){
  ...
  function T(a) {
    U(a, function(a) {
      var b = f.G("Pageshot-by-bsquochoai.ga-");
      v[b] = a;
      chrome.tabs.create({url:chrome.extension.getURL(caidat.pluginfolder+"pageshot.html?id=" + b)}, function(a) {
  ...
  return{R:function() {
	...
	if (cmd == "plugins_pageshotbybsquochoai"){
		 s();
	}
	});
	chrome.runtime.onMessage.addListener(
		function(req, sender, sendResponse) {
			if(req.ham == "talktochrome" && req.lenh == "màn hình"){
				s()
			}
	})
