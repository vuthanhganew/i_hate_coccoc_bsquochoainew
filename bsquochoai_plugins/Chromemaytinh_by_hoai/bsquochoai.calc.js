...
var caidat = {
	host: "http://web2.0calc.com"
}

// (c) 2006-2015 @@ Trần Quốc Hoài bsquochoainew@gmail.com    
...
Calc.prototype.loopAnim = function() {
	...
	window.requestAnimFrame(function(){self.loopAnim();});	 
};
		...
		if (window.location) {
			...
			if (location.indexOf("#")>0) {
				...
				if (urlinput.match("[0-9]")) {
					...
					var inp = $("<input name=\"url_input\"></input>");
					inp.val( urlinput );
					inp.appendTo(form);
					form.appendTo($("body"));
					form.submit();					
...
Calc.prototype.init = function(){
	...
	if (self.iswidget) {
		...
		{
			self.display.minheight = self.display.height = 10;
		}
	}
...
Calc.prototype.clickbutton = function(event, btn) {
	...
	switch(id){
		...
		case "BtnDiv": shift?self.input.ins(" mod "): self.input.setoperation('/'); break;
		case "BtnPowXY": self.input.setoperation('^'); break;
		case "BtnDot": self.input.ins(".", btn); break;
		case "BtnSign": self.input.sign(); break;
		...
		case "BtnSqrt": self.input.cmd('sqrt', btn); break;
		case "BtnClear": self.input.clearall(); break;
		...
		case "BtnBack": self.input.back(); break;
		case "BtnLeft": self.input.movecaret((event.pageX-btn.offset().left)>btn.width()/2); break;
		...
		case "BtnVar": 
			if (shift){
				var varscontextmenu=$("#varscontextmenu");
				varscontextmenu.css({"left":(event.pageX+"px"),"top":(event.pageY+"px")});		  	  
				varscontextmenu.show();
				
				varscontextmenu.find("a").click(function(event){
					varscontextmenu.find("a").unbind("click");
					$(document).unbind("click");
					varscontextmenu.hide();
					event.stopPropagation();
					event.preventDefault();
					self.input.ins( $(this).text() );
					self.preview.input = self.input.val();	
					
				});
				
				$(document).bind("click", function(event){
					if ("BtnVar"!=$(event.target).attr("id") && !$(event.target).parent("ul").hasClass("dropdown-menu"))
		  			{
						$(document).unbind("click");
						varscontextmenu.hide();
		  			}					
		                	 ...
		                	 try{
			                	 ...
			                	 for (var i=0;i<results.length;i++)
			    	    		 {
			    	        		 ...
			    	        		 switch (r.status) {
			    	        		 	...
			    	        		 	case "overloaderror":
												alert("Lỗi, chờ xíu thử lại nhen em.");
			    	        			 break;
...
var func_resize = function(){
	...
	if (widget)
		$("div#display").css( {height:(($(window).height() - $("div.center").height() -20 -($("div.navbar").length>0?$("div.navbar").height():0) )+"px")} );
...
History1.prototype.add = function(_in, _out, cl) {
	var self = this;
	...
	var li = $("<li"+(cl?" class=\""+cl+"\"":"")+"><p title=\""+_out+"\" class=\"r\">"+r+"</p><p title=\""+_in+"\" class=\"l\">"+l+"</p></li>");
...
History1.prototype.init = function() {
	...
	this.frame = this.histories.find("div").eq(0);
	...
	this.histories.find(".last").click(function(event){
		self.input.val( $(this).attr("title") );
		event.stopPropagation();
		event.preventDefault();
	});
...
Displaycontextmenu.prototype.init = function() {
	...
	this.menu.find("a").click(function(event){
		...
		if (a.hasClass("permalink")) {
			...
			var div = $("<div></div>");
			div.load(caidat.host+"/js/templates/permalink.jsp", function(){
		...
		var self = this;
		 this.inputhelpermenu.find("a").click(function(event){
			 ...
			 if (a.hasClass("unit")) {
				 ...
				 helperdiv.load("template/unit.html", function(){
					 helperdiv.show();
			 ...
			 if (a.hasClass("fraction")) {
				 ...
				 helperdiv.load("template/fraction.html", function(){
					 helperdiv.show();
			 ...
			 if (a.hasClass("solver")) {
				 ...
				 helperdiv.load("template/solver.html", function(){
					 helperdiv.show();
			 ...
			 if (a.hasClass("vector")) {
				 ...
				 helperdiv.load("/js/tmpl/vector.html", function(){
					 ...
					 helperdiv.find("select.opa").click(function(){
						 precalcfunc();
						});
					 helperdiv.find("select.opb").click(function(){
						 precalcfunc();
						});
					 helperdiv.find("select.opaa").click(function(){
						 precalcfunc();
						});
					 helperdiv.find("select.opbb").click(function(){
						 precalcfunc();
						});
					 
					helperdiv.find("select.type").click(function(){
						changetype( $(this) );
					});
					
					 helperdiv.find("select.vector").click(function(){
						 updatevector( $(this).parent() );
					 });
					 helperdiv.find("select.matrix").click(function(){
		 ...
		 this.inputhelper.click(function(event){
			 var glyphicon = $("#inputhelper span.glyphicon");
...
function Graph(){
	...
	this.modal = null;	
	this.loading = null;
	this.minmax = [null,null,null,null];
	this.center = null;
	this.localname = "tmp:graph";
	this.axisbounds = [-360,360,-2,2];
	this.graphinputs = [];
	this.trigodeg = null;
	this.trigorad = null;
	this.loadurl = "template/graph.html";
	this.type = "graph";
	this.minrowcount = 2;
}

Graph.prototype.savetemp = function(){
	
	var title = this.modal.find("input.title").val();
	var rows = [];
	
	this.modal.find(".edit ul.rows:first li.row").each(function(index){
		var li = $(this);			
	...
	var data =JSON.stringify( { rows:rows, title:title, trig:(this.trigodeg.is(':checked') ?"deg":"rad"), bounds:this.axisbounds, time:new Date().getTime() } );
...
Graph.prototype.cleartemp = function() {
	try{
	    ...
	    localStorage.setItem( this.localname , null);
    
	} catch (e) {
	}
};

Graph.prototype.loadtemp = function() {
	...
	try{
	    ...
	    var data = localStorage.getItem( this.localname );
	    if (!data)
	    	return;
	    ...
	    if (data.bounds) {
	    	for (var i=0;i<4 && i<data.bounds.length;i++)
	    		self.axisbounds[i] = data.bounds[i];
	    }
	    
	    if (data.trig=="rad") {
	    	if (this.trigorad && this.trigodeg) {
	    		this.trigodeg.removeAttr("checked");
	    		this.trigorad.attr("checked", true);
	    	}	    		
	    }
	    if (data.trig=="deg") {
	    	if (this.trigorad && this.trigodeg) {
	    		this.trigorad.removeAttr("checked");
	    		this.trigodeg.attr("checked", true);
	    	}	    		
	    }
	    
	    this.modal.find("input.title").val( data.title );
	    
	    var rows = this.modal.find(".edit ul.rows:first li.row");
		
		for (var i=rows.length;i<data.rows.length || i<self.minrowcount;i++)
			this.addRow();
		
		rows = this.modal.find(".edit ul.rows:first li.row");
		...
		rows.each(function(index){
			...
			if (index < data.rows.length) {
				...
				var val = data.rows[index];
				//li.find(".id").text( val[0] );
				li.find("input.inp").val( val[1] );
				
			} else {
				if (index>1)
					li.find("input.inp").val("");
			}
...
Graph.prototype.refresh = function(){
	...
	self.axisbounds[0] = self.round( self.axisbounds[0] );
	self.axisbounds[1] = self.round( self.axisbounds[1] );
	self.axisbounds[2] = self.round( self.axisbounds[2] );
	self.axisbounds[3] = self.round( self.axisbounds[3] );
	...
	var graph = this.modal.find(".graph");
	var params = "options="+encodeURIComponent( JSON.stringify( { r:rows, trig:(self.trigodeg.is(':checked') ?"deg":"rad"), b:self.axisbounds, w:graph.width(), h:graph.height(), c:Math.random() } ) );
	
	this.loading.show();
	var bgimg = $("<img />");
	var url = caidat.host+"/graph?"+params;
	bgimg.load(function(){
	  graph.css({backgroundPosition:"0px 0px", backgroundImage:("url(\""+url+"\")") } );
	  self.loading.hide();
	});
	bgimg.attr("src", url);
	
	for (var i=0;i<4;i++)
		self.minmax[i].val( self.axisbounds[i] );
	
	self.center.css({left:((graph.width()/2-2)+"px"), top:((graph.height()/2-2)+"px") });
...
Graph.prototype.load = function(){
	...
	if (self.graphinputs==null)
		return;
	if (self.graphinputs.length==0)
		return;
	
	for (var i=rows.length;i<self.graphinputs.length;i++)
		this.addRow();
	...
	rows.each(function(index){
		...
		if (index < self.graphinputs.length) {
			
			li.find("input.inp").val( self.graphinputs[index] );
...
Graph.prototype.round = function(val) {
 	var tmp = Math.round( val * 1000.0 )/1000.0;
	if (isNaN(tmp))
		return val;
	return tmp;
};

Graph.prototype.prepare = function(div){

	var self = this;
	
	
		
	var modal = self.modal = div;
	
	self.center = modal.find(".graph .center");
	
	self.loading = modal.find(".loading");
	
	{
		var mm = modal.find(".minmax span input");
		self.minmax=[ mm.eq(0), mm.eq(2), mm.eq(3), mm.eq(1) ];			
		...
		var f = function(){
			for (var i=0;i<4;i++) {
				try{
					var val = self.round(  self.minmax[i].val()  );
					self.minmax[i].val( val );
					self.axisbounds[ i ] = parseFloat( val );
				}catch(e){}
			}
			self.refresh();
		};
	...
	var graphdiv = modal.find(".graph");
	graphdiv.resize(function(){
					
		self.refresh();
	});
	
	modal.find("button.refresh").click(function(){
		self.refresh();
	});
	
	
	var zoomctl = modal.find(".zoomctl button.btn-sm, .zoomctl a.btn-sm");
	
	  var btn_left = zoomctl.eq(0);
	  var btn_right = zoomctl.eq(1);
	  var btn_top = zoomctl.eq(2);
	  var btn_bottom = zoomctl.eq(3);
	  
	  var btn_zoominh = zoomctl.eq(4);
	  var btn_zoomouth = zoomctl.eq(5);
	  var btn_zoominv = zoomctl.eq(6);
	  var btn_zoomoutv = zoomctl.eq(7);
	  
	  var btn_zoomin = zoomctl.eq(8);
	  var btn_zoomout = zoomctl.eq(9);

	  	  
	  var btn_reset = zoomctl.eq(10);
	  	  	  
	
  	  
	  
	  
	  btn_zoominh.click(function(event){
		  var dx = (self.axisbounds[1]-self.axisbounds[0])/4;
		  self.axisbounds[0]+=dx;
		  self.axisbounds[1]-=dx;
		  self.refresh();
		  event.stopPropagation();        
		  return false;
	  });
	  btn_zoomouth.click(function(event){
		  var dx = (self.axisbounds[1]-self.axisbounds[0])/4;
		  self.axisbounds[0]-=dx;
		  self.axisbounds[1]+=dx;
		  self.refresh();
	  ...
	  btn_zoominv.click(function(event){		  
		  var dy = (self.axisbounds[3]-self.axisbounds[2])/4;
		  
		  self.axisbounds[2]+=dy;
		  self.axisbounds[3]-=dy;
		  self.refresh();
		  event.stopPropagation();        
		  return false;
	  });
	  btn_zoomoutv.click(function(event){		  
		  var dy = (self.axisbounds[3]-self.axisbounds[2])/4;
	  ...
	  btn_zoomin.click(function(event){
		  var dx = (self.axisbounds[1]-self.axisbounds[0])/4;
		  var dy = (self.axisbounds[3]-self.axisbounds[2])/4;
		  self.axisbounds[0]-=dx;
		  self.axisbounds[1]+=dx;
	  ...
	  btn_zoomout.click(function(event){
		  var dx = (self.axisbounds[1]-self.axisbounds[0])/4;
		  var dy = (self.axisbounds[3]-self.axisbounds[2])/4;
		  self.axisbounds[0]+=dx;
		  self.axisbounds[1]-=dx;
	  ...
	  btn_reset.click(function(event){
		  ...
		  if (self.trigodeg.is(':checked') )
			  self.axisbounds = [-360,360,-2,2];
		  else
			  self.axisbounds = [-8,8,-2,2];
		  
		  graphdiv.css({width:"100%", height:"320px"});
		  self.refresh();		  
		  event.stopPropagation();        
		  return false;
	  });
	  ...
	  graphdiv.bind('DOMMouseScroll',function(event){
    	  
    	  if (event.originalEvent)
    	  {
	    	  var delta = event.originalEvent.detail<0?1:-1;
			  var dx = delta* (self.axisbounds[1]-self.axisbounds[0])/4;
			  var dy = delta* (self.axisbounds[3]-self.axisbounds[2])/4;
			  self.axisbounds[0]+=dx;
			  self.axisbounds[1]-=dx;
			  self.axisbounds[2]+=dy;
			  self.axisbounds[3]-=dy;
	  ...
	  graphdiv.bind('mousewheel', function(event){
 ...
 
      {
		  ...
		  graphdiv.mousedown(function(event){				  
			  ...
			  mousey = y;
			  mousedown = true;
			  if (x<graphdiv.width()-10 && y<graphdiv.height()-10) {					
				event.stopPropagation();
			  	return false;
			  }
		  } );        		          		  
		  graphdiv.mouseup(function(event){
			  ...
			  var fx = (self.axisbounds[1]-self.axisbounds[0])/graphdiv.width();
			  var fy = (self.axisbounds[3]-self.axisbounds[2])/graphdiv.height();
			  
			  self.axisbounds[0]-=dx*fx;
			  self.axisbounds[1]-=dx*fx;
			  self.axisbounds[2]+=dy*fy;
			  self.axisbounds[3]+=dy*fy;
		  ...
		  graphdiv.mousemove(function(event){
			  if (mousedown)
			  {
			   var x = event.clientX;
			   var y = event.clientY;	
			   ...
			   graphdiv.css({"backgroundPosition":( dx+"px "+dy+"px" )});
			   if (x<graphdiv.width()-10 && y<graphdiv.height()-10) {
     ...
     var ul = modal.find(".list ul.rows:first");
  	
     ul.children("li").each(function(){
    	 	var li = $(this);
    	 	
    	 	var ctrls = li.find(".ctrls");
    	 	if (ctrls.length>0) {
    	 		ctrls.find("button.up").click(function(){ self.moveup( $(this).parents("li:first") ); });
    	 		ctrls.find("button.down").click(function(){ self.movedown( $(this).parents("li:first") ); });
    	 		ctrls.find("button.delete").click(function(){ 
...
Graph.prototype.moveup = function(row){
	if (row.children().length>1)
		row.insertBefore( row.prev() );
	
	this.refresh();
};
Graph.prototype.movedown = function(row){
	if (row.children().length>1)
		row.insertAfter( row.next() );
	
	this.refresh();
};
Graph.prototype.remove = function(row){
...
Graph.prototype.prepareMoveable = function(){
	...
	var movefct = function(top, ul) {
		...
		if (moveable!=null) {
			...
			newi = parseInt( (top-10) / ( firstli.height()+padding ) );
			if (newi<0)
				newi = 0;
						
			if (top < 6) {
				var l = moveable.children("li:first");
				tmp[i] = l;
				i++;
				y+=l.height()+padding;
				newi = -1;
		...
		ul.find("li").each(function(){
			...
			y+=l.height()+padding;
			if (moveable!=null && i==newi) {
				tmp[i] = l;
				y+=l.height()+padding;
				i++;
				tmp[i] = moveable.children("li:first");
				inserted = true;
			} else {
				tmp[i] = l;
			}
		...
		if (!inserted && (!moveable.hasClass("deleteable") || i<1 )) {
			tmp[i] = moveable.children("li:first");
			y+=tmp[i].height()+padding;
			i++;
		}
	...
	var idfct = function(id, event){
		
		ul.css({minHeight:( ul.height()+ "px")});
		
		moveable = $("<ul class=\"rows\"></ul>");				
		var li = id.parents("li:first");
		li.parent().addClass("moving");
		
		var top = lasttop = -14+event.pageY - modal.find(".list").offset().top;
		
		movefct( top, li.parent() );
		
		var width = li.width();
		li.remove();
		li.appendTo(moveable);
		
		
		moveable.css({left:"14px", top:(top+"px")});
		moveable.width( width-40 );
		moveable.addClass("moveable");
		modal.addClass("moveable");
		moveable.appendTo( modal.find(".list .edit") );
	...
	var firstrow = modal.find(".row:first");
	
	/*
	firstrow.mousemove(function(event){
		
		var row = $(this);			
		if (moveable!=null) {
			var top = -14+event.pageY - row.offset().top;
			
			var maxtop = modal.find(".rowctl").offset().top - ul.offset().top - 10;
			...
			if (top>maxtop) 
				moveable.addClass("deleteable");
			else
				moveable.removeClass("deleteable");
			
			moveable.css({top:(top+"px")});
			movefct( top, ul );
	...
	firstrow.mouseup(function(event){
		if (moveable!=null) {
			moveable.remove();
			
			var li = moveable.find("li");				
			//li.appendTo( modal.find(".list ul.rows") );
			ul.children().remove();
			for (var i=0;i<sortedlis.length;i++)
			{
				sortedlis[i].find(".id").text( self.getRowId(i) );
				sortedlis[i].appendTo(ul);
			}
			modal.find(".list ul.rows").removeClass("moving");
			
			ul.find(".id").each(function(){
				
				var _id = $(this);
				_id.unbind("mousedown");
				_id.mousedown(function(event){
					idfct(_id, event);
				});
				
			});
			
			modal.removeClass("moveable");
			ul.css({minHeight:"30px"});
...
Graph.prototype.show = function(){
	...
	if (modal.length==1) {
		...
		if (modal.hasClass("type-"+self.type)) {
	...
	div.load( self.loadurl, function(){
		...
		self.prepare( self.modal );
		self.prepareMoveable();
		
		self.modal.appendTo($("body"));
		
		$("#modal").addClass("type-"+self.type);
		
		
		self.modal.find("input.inp").keyup(function(event){
			if (event.keyCode==13) {
				self.refresh();
			}
		});
		
		self.modal.find(".addline").click(function(){
			self.addRow();			
		});
		...
		self.modal.find(".fullscreen").click( function() {
			self.savetemp();
			self.modal.modal('hide');
			window.open( self.modal.find(".fullscreenurl").val() );
		});
...
Graph.prototype.getRowId = function( i ) {
	return String.fromCharCode( 102+ i )+"(x)";
};
...
Graph.prototype.addRow = function() {
	...
	var ul = this.modal.find(".list ul.rows:first");
	if (ul.children("li").length>25) {
		alert("info: limit reached!");
		return;
	}
	
	var li = ul.children("li:last").clone(true);
	li.find("input").val("");
	li.find(".id").text( this.getRowId( ul.find("li").length ) );
	li.appendTo(ul);
};
...
Graph.prototype.init = function() {
	...
	if (self.graph)
	self.graph.click(function(event){		
		self.show();
		event.stopPropagation();
		event.preventDefault();
	});
...
Calclet.prototype = new Graph();
Calclet.prototype.constructor = Calclet;

function Calclet() {
	this.loadurl = "/js/tmpl/calclet.html";	
	this.type = "calclet";
	this.localname = "tmp:calclet";
	this.minrowcount = 5;
}

Calclet.prototype.refresh = function(){	
	
};


Calclet.prototype.getRowId = function( i ) {
	return "L"+i;
};

Calclet.prototype.prepare = function(div){	
	var self = this;
	
	self.modal = div;
	
	self.loading = self.modal.find(".loading");
	...
	var rows = self.modal.find(".edit ul.rows:first li.row");
	rows.each(function(index){
		var li = $(this);
		
		li.keyup(function(event){
			if (event.keyCode!=13) {
				self.updatePreviewImg( li.find("input.inp") );
				li.find("input.out").val("");
			}
		});
...
Calclet.prototype.updatePreviewImg = function(inp){
		
	var self = this;
	
	var parameters = {"in": [ inp.val() ], trig:(self.trigodeg.is(':checked') ?"deg":"rad") };		
	
	var previewimg = inp.parents("li:first").find("img.result:first");
...
Calclet.prototype.refresh = function(){
	...
	rows.each(function(index){
		var li = $(this);
		var val = li.find("input.inp");
				
		inp.push( val.val() );			
	});
	
	var parameters = {"in": inp, trig:(self.trigodeg.is(':checked') ?"deg":"rad"), p:displayoptions.precision, s:displayoptions.scientific };
	...
	$.ajax({
         ...
         success: function (json) {
        	 ...
        	 for (var i=0;i<results.length && i<rows.length;i++)
    		 {
        		 var r = results[i];
        		 
        		 var row = rows.eq(i);
        		 var resultimg = row.find("img.result");
        		 var outputinput = row.find("input.out");
        		 
        		
        		 if (r.graph) {
 		 			var src = r.graph;	         		 			
 		 			resultimg.attr("src",src);	         		 			
 		 			resultimg.attr("title", inp);
 		 		 }
        		 	...
        		 	case "ok":
        		 		...
        		 		if (r.out) {
        		 			...
        		 			if (r.out=="")
        		 				...
        		 				outputinput.val( r.out.indexOf("=")<0?( "= "+r.out ):( r.out ) );        		 				    	       
...
Sketch.prototype = new Graph();
Sketch.prototype.constructor = Sketch;

function Sketch() {
	this.loadurl = caidat.host+"js/modal/sketch.jsp";	
	this.type = "sketch";
	this.localname = "tmp:sketch";
	this.minrowcount = 5;
	
	this.id = null;
	
	this.axisbounds = [-10,10,-10,10];
}

 
Sketch.prototype.getRowId = function( i ) {
	return "S"+i;
};

 
Sketch.prototype.savetemp = function(){
	
	var self = this;
	
	 var edit = self.modal.find("div.edit");
	 	
	 	var fields = [];
	 	
	 	edit.children("ul.rows").children("li.row").each(function(){
	 		var row = $(this);
	 		var id = row.find("span.id").text();
	 		var label = row.find("input.lbl").val();
	 		var textcolor = row.find("span.line.color").text();
	 		var backgroundcolor = row.find("span.background.color").text();
	 		var type = row.find("select.type:first").val();
	 		
	 		var prop = row.find(".property");
	 		
	 		var childs = {};
	 		prop.children("ul.type."+type).each(function(){
	 			var ul = $(this);
	 			...
	 			ul.find("input,select,textarea").each(function(){
	 				...
	 				if (name!=null && name.length>0 && elem.val()!=null && elem.val().length>0)
	 				{					
	 					childs[ name ] = encodeURIComponent(elem.val());		
	 				}
	 			});
	 		...
	 		fields.push( {id:id, label:label, colors:[textcolor, backgroundcolor], selected:type, fields:childs } );
	 	});
	 	
	 	var json =  JSON.stringify({fields:fields, axisbounds:self.axisbounds, time:new Date().getTime() });
	 	
	 	 try{
				if (('localStorage' in window && window['localStorage'] !== null)) {		    				
				    if (localStorage) { 	
				    		localStorage.setItem("current:sketch",  json  );
				    }
				}
			} catch (error) {
			}
...
Sketch.prototype.loadtemp = function(json) {
	
	var self = this;
	
	try{
		if ((json) || (('localStorage' in window && window['localStorage'] !== null))) {		    				
		    if (localStorage || json) { 	
		    	
		    	if (!json)
		    	 json = localStorage.getItem("current:sketch");
		    	
		    	if (json) {
		    		json = JSON.parse( json );
		    		var fields = json.fields;
		    		self.axisbounds = json.axisbounds;
		    		...
		    		{
		    			var li = self.modal.find("div.edit ul li.row.active:first");
		    			var ul = li.parent();
		    			ul.children("li.row").removeClass("active");
		    			li = li.clone(true);
		    			li.appendTo(ul);	
		    			li.attr("id","row"+ rowcount );
		    			
		    			li.addClass("active");
		    			 
		    			prop = li.find("div.property");
		    			prop.attr("id","prow"+ rowcount );
		    		...
		    		var i=0;
		    		self.modal.find("div.edit").children("ul.rows").children("li.row").each(function(){
		    	 		var row = $(this);
		    	 		
		    	 		var field = fields[i];
		    	 		if (field) {
		    	 			var typeselect = row.children("select.type:first");
			    	 		typeselect.val(field.selected);
			    	 		changeDrawType( typeselect );
			    	 		
			    	 		row.children("span.id").text( field.id );
			    	 		row.children("input.lbl").val( field.label );
			    	 		row.children("span.line.color").text( field.colors[0] );
			    	 		row.children("span.color.background").text( field.colors[1] );
			    	 					    	 		
			    	 		for (var name in field.fields) {
			    	 			row.find("input[name='"+name+"'],select[name='"+name+"'],textarea[name='"+name+"']").val( decodeURIComponent(field.fields[name]) );				    	 			
			    	 		}
			    	 		row.find("select.subtype").each(function(){
			    	 			selectsubtype( $(this) );
			    	 		});
		    	 		}	
...
var idslabel = {"point":"P", "line":"L", "straight":"S", "verticalline":"L", "verticalstraight":"S", "circle":"C", "angle":"A", "vector":"V", "number":"N", "comment":"", "triangle":"T" };

Sketch.prototype.prepareMoveable = function(){
};
 
Sketch.prototype.prepare = function(div){

	var self = this;
	var modal = self.modal = div;
	Graph.prototype.prepare.call(this, div);
	
	var graphdiv = modal.find(".graph");
	
	modal.find("input").keyup(function(){
		graphdiv.css({opacity: 0.3});
	});
	
	var zoomctl = modal.find(".zoomctl button.btn-sm, .zoomctl a.btn-sm");
	 var btn_reset = zoomctl.parent().find(".reset");
	 
	  var btn_zoomin = zoomctl.eq(4);
	  var btn_zoomout = zoomctl.eq(5);
	 
	  btn_zoomin.unbind("click");
	  btn_zoomout.unbind("click");
...
Sketch.prototype.rowcount = 2;

Sketch.prototype.addRow = function(){
...
var li = self.modal.find("div.edit ul li.row.active:first");
	...
	if (li.length==0)
		li = self.modal.find("div.edit ul li.row:last");
		
	var ul = li.parent();
	ul.children("li.row").removeClass("active");
	
	var firstselect = li.find("select:first");
	var rowtype = firstselect.val(); 
	
	// props
	var prop = li.find("div.property");
	prop.find("select").each(function(){
		var s = $(this);
		s.data( "val", s.val() );
	});
	...
	firstselect = li.find("select:first");
	firstselect.val( rowtype );
	
	
		
	li.find("input[name='result']").val("");
	li.find(".jPicker").remove();
	
	li.attr("id","row"+ self.rowcount );
	
	li.addClass("active");
 
	prop = li.find("div.property");
	prop.attr("id","prow"+ self.rowcount );
	
	
	changeDrawType( firstselect );
...
function colorPicker(sketch, node)
{
		node.find('span.color').each(function(){
		
		var span = $(this);
		
		
		//var color = span.hasClass("line")?(rndColor()+'c0'):( rndColor() +'20');
		var color = span.text();
		...
		span.jPicker(
		        {
			          ...
			          {
			            alphaSupport: true,
			            active: new $.jPicker.Color({ a:parseInt(color.substring(0,2), 16), hex: color.substring(2) })
			          },
			          localization: 
			          {
			            text:
			        	...
			        	var a = (all && all.a.toString(16) || '00');
			        	var hex = (all && all.hex || '000000');
			        	if (hex=="ffffff")
			        		hex="fffffe";
		...
		node.find(".jPicker:last span.Image").attr("title", span.attr("title"));
		...
		$(".Icon").click(function(){
			...
			$("td.Button input.Cancel").click(function(){
				sketch.modal.show();
				$(".modal-backdrop").show();
			});
		});
...
function selectrow( row ){
	...
	$("div.edit li.row").removeClass("active");
	row.addClass("active");
	 
	
}

function selectsubtype( select ){
	 
	
	select.parent("li").children('ul.active').removeClass("active");
	select.parent("li").children('ul.'+select.val()).addClass("active");
}

function changeDrawType(select){
	var type = select.val();
	...
	if (type=="") {
		type="circle";
		window.setTimeout(function(){select.val("circle");}, 1000);		
	}
	...
	var pli = div.find(".property");
	
	pli.find("ul.type").hide();
	pli.find("ul.type."+type).show();
	...
	switch (type) {
		...
		case "straight":
			...
			div.find(".jPicker:first").show();
			div.find(".jPicker:last").hide();
		break;
		default:
			
			div.find(".jPicker:first").show();
			div.find(".jPicker:last").show();
		break;
	}

	
	var li = div;
	var row = parseInt(li.attr("id").replace('row',''));
	...
	switch (type) {
		...
		break;
		default:
			div.find(".id").text( idslabel[type]+row );
		break;
	}
	/*
	{
		var ids = 0;		
		$("#edit").find("select.type").each(function(){
			var t = $(this).val();
			ids++;
			$(this).parent().find(".id").text( idslabel[t]+ (ids) );
		});				
	}
	*/
	...
	var rowindex = 0;
	select.parents("div.edit:first").find("li.row").each(function(){
		
		var row = $(this);
		row.find("select.type").each(function(){
			
			var id = row.find(".id").text();
			var label = row.find(".lbl").val();
			
			var tmp = [ id, label, rowindex ];
			switch ($(this).val()) {
				case "point":
					points.push( tmp );						
				break;		
	...
	select.parents("div.edit:first").find("li.row").each(function(){
		...
		row.find("select").each(function(){
			...
			var arr = null;		
			if (select.hasClass("allpoints"))
				arr = points;
			else
			if (select.hasClass("allcircles"))
				arr = circles;
			else
			if (select.hasClass("alllines"))
				arr = lines;
			if (select.hasClass("allvariables"))
				arr = variables;
			if (select.hasClass("allvectors"))
				arr = vectors;
			if (select.hasClass("allnumbers"))
				arr = numbers;
			...
			if (arr!=null) {
				...
				for (var i=0;i<arr.length;i++){
					...
					if (arr[i][2] < rowindex)
						select.append($("<option></option>").attr("value",id).text(arr[i][0]+((arr[i][1].length>0)?(": "+arr[i][1]):"") ));			
				};
...
Sketch.prototype.loaddeps = function() {
	var self = this;
	
	$.getScript("js/jpicker-1.1.6/jpicker-1.1.6.min.js",
				...
				function() {
						...
						$(".jPicker").remove();
						edit.find("li.row").each(function(){
							var li = $(this);
							colorPicker( self, li );
							
							li.find("select.type:first").each(function(){
								changeDrawType( $(this) );	
							});
							
							li.find("select.subtype").each(function(){
								selectsubtype( $(this) );	
							});
	...
	$("<link/>",{rel:"stylesheet",type:"text/css",href:"css/"+"jpicker-1.1.6/jpicker-1.1.6.min.css"}).appendTo("head");
	this.loaddeps = function(){};
};
	
Sketch.prototype.refresh = function(){
	
var self = this;

	self.loaddeps();
	
	var clearinput = function (inp) {
		inp = $.trim(inp);
		if (inp.length==0)
			inp="0";
		return inp;
	...
	self.modal.find("div.edit").css({height:( self.modal.find("div.graph").height() +"px")});
	...
	self.modal.find("div.edit ul li.row").each(function(index){
		var li = $(this);
		...
		//var div = li.find("ul."+type);
		
		var property = li.find(".property");
		var div = property.find("ul."+type);
		
		
		var id = li.find(".id").text();

		var colors = li.find("span.color.line").text()+","+ li.find("span.color.background").text();
		var label = $.trim(li.find(".lbl").val().replace("\"","'"));
		var style = ",{"+(label.length==0?0:("\""+label+"\""))+","+colors+"}"; 
		...
		switch (type) {
			...
			case "number":
			var numbertype = div.find("select[name='number.type']").val();
			
			var ul = div.find("ul."+numbertype);
			
			switch(numbertype) {
				case "v":
					var value = ul.find("input[name='number.v']").val();
					rows.push( id+"=("+clearinput(value)+")" );
				break;
			...
			case "vector":
			var vectortype = div.find("select[name='vector.type']").val();
			
			var ul = div.find("ul."+vectortype);
			
			switch(vectortype) {
				case "v":
					var value = ul.find("input[name='vector.v']").val();
					rows.push( id+"=("+clearinput(value)+")" );
				break;
				case "xy":
					var x = ul.find("input[name='vector.x']").val();
					var y = ul.find("input[name='vector.y']").val();
					rows.push( id+"=("+clearinput(x)+", "+clearinput(y)+")" );
				break;			
			}
			break;
		
			case "angle":
				var angle = div.find("select[name='angle.type']").val();
				var index = div.find("select[name='angle.index']").val();
				
				var ul = div.find("ul."+angle);
				
				switch(angle) {
					case "v":
						var alpha = ul.find("input[name='angle.v']").val();
						rows.push( id+"=index(angle(("+clearinput(alpha)+")"+style+")"+","+index+")");
					break;
					case "a":
						var alpha = ul.find("select[name='angle.a']").val();
						rows.push( id+"=index(angle("+alpha+" "+style+")"+","+index+")");
					break;
					case "ll":
						var l0 = ul.find("select[name='angle.ll.a']").val();
						var l1 = ul.find("select[name='angle.ll.b']").val();
						rows.push( id+"=index(angle("+l0+", "+l1+" "+style+")"+","+index+")");
					break;
					case "lp":					
						var l0 = ul.find("select[name='angle.lp.a']").val();
						var p0 = ul.find("select[name='angle.lp.b']").val();
						rows.push( id+"=index(angle("+l0+", "+p0+" "+style+")"+","+index+")");
					break;
					
					case "pl":					
						var p0 = ul.find("select[name='angle.pl.a']").val();
						var l0 = ul.find("select[name='angle.pl.b']").val();
						rows.push( id+"=index(angle("+p0+", "+l0+" "+style+")"+","+index+")");
					break;
										
					case "ppp":
						var p0 = ul.find("select[name='angle.ppp.a']").val();
						var p1 = ul.find("select[name='angle.ppp.b']").val();
						var p2 = ul.find("select[name='angle.ppp.c']").val();
						
						rows.push( id+"=index(angle("+p0+", "+p1+", "+p2+" "+style+")"+","+index+")");
					break;
			...
			case "circle":
				...
				var center = div.find("select[name='circle.center']").val();
				var ul = div.find("ul."+center);
				switch (center)
				{
					case "v":
						var x = ul.find("input[name='circle.x']").val();
						var y = ul.find("input[name='circle.y']").val();
						xy = "(("+clearinput(x)+"),("+clearinput(y)+"))";
					break;
					case "p":
						xy = ul.find("select[name='circle.p']").val();
					break;					
				}
				
				
				var radius = div.find("select[name='circle.radius']").val();
				ul = div.find("ul."+radius);
				switch (radius)
				{
					case "v":
						r = ul.find("input[name='circle.r']").val();
						r = "("+clearinput(r)+")";
					break;
					case "n":
						r = ul.find("input[name='circle.n']").val();
						r = "("+clearinput(r)+")";
					break;
					case "dp":
						r = ul.find("select[name='circle.dp']").val();						
						r = "length("+ xy +"-"+ r+")";						
					break;					
				}
						
				rows.push( id+"=circle("+xy+", "+r+style+")" );
			...
			case "line":
				...
				var linetype = div.find("select[name='line.linetype']").val();				
				div = div.find("ul."+linetype);
				...
				switch (linetype) {
					...
					var start = div.find("select[name='line.ls.start']").val();
					var ul = div.find("ul."+start);
					switch (start)
					{
						case "v":
							x0 = ul.find("input[name='line.ls.x0']").val();
							y0 = ul.find("input[name='line.ls.y0']").val();
							xy0 = "(("+clearinput(x0)+"),("+clearinput(y0)+"))";
						break;
						case "p":
							xy0 = ul.find("select[name='line.ls.p0']").val();
						break;
						
					}
						
					var end = div.find("select[name='line.ls.end']").val();
					ul = div.find("ul."+end);
					switch (end)
					{
						case "v":
							x1 = ul.find("input[name='line.ls.x1']").val();
							y1 = ul.find("input[name='line.ls.y1']").val();
							xy1 = "(("+clearinput(x1)+"),("+clearinput(y1)+"))";
						break;
						case "p":
							xy1 = ul.find("select[name='line.ls.p1']").val();
						break;					
					}
						
					rows.push( id+"="+linetype+"("+xy0+", "+xy1+""+style+")" ); 
					...
					case "os":
						...
						var start = div.find("select[name='line.olos.start']").val();
						var ul = div.find("ul."+start);
						switch (start)
						{
							case "l":
								l0 = ul.find("select[name='line.olos.l0']").val();
							break;
							
						}
								
						var end = div.find("select[name='line.olos.end']").val();
						ul = div.find("ul."+end);
						switch (end)
						{
							case "v":
								x1 = ul.find("input[name='line.olos.x1']").val();
								y1 = ul.find("input[name='line.olos.y1']").val();
								xy1 = "(("+clearinput(x1)+"),("+clearinput(y1)+"))";
							break;
							case "p":
								xy1 = ul.find("select[name='line.olos.p1']").val();
							break;					
						}
							
						rows.push( id+"="+linetype.replace("o","")+"("+xy1+", vertical("+l0+", "+xy1+")"+style+")" ); 
			...
			break;			
				...
				case "triangle":
					...
					for (var i=0;i<3;i++) {
						...
						var position = div.find("select[name='triangle.position"+i+"']").val();
						var ul = div.find("ul."+position);
						...
						switch (position)
						{
							case "v":
								...
								x = ul.find("input[name='triangle.x"+i+"']").val();
								y = ul.find("input[name='triangle.y"+i+"']").val();
								
								points[i] = "(("+clearinput(x)+"),("+clearinput(y)+"))"; 
							break;
							
							case "p":
								
								var p = ul.find("select[name='triangle.p"+i+"']").val();
								
								points[i] = "("+clearinput(p)+")"; 
							break;
						}
					
					}
					
					rows.push( id+"=triangle("+ points[0]+", "+points[1]+", "+points[2] +style+")" ); 
			break;
			 
			case "point":
				
				var position = div.find("select[name='point.position']").val();
				var ul = div.find("ul."+position);
				...
				switch (position)
				{
					case "v":
						...
						x = ul.find("input[name='point.x']").val();
						y = ul.find("input[name='point.y']").val();
						
						rows.push( id+"=point(("+clearinput(x)+"),("+clearinput(y)+")"+style+")" ); 
					break;
					
					case "p":
						
						var p = ul.find("input[name='point.p']").val();
						
						rows.push( id+"=point(("+clearinput(p)+")"+style+")" ); 
					break;
					...
					case "ml":
						...
						var l = ul.find("select[name='point.ml']").val();
						rows.push( id+"=point( midpoint("+l+") "+style+")" );
					...
					case "mc":
						...
						var c0 = ul.find("select[name='point.mc']").val();
						rows.push( id+"=point( midpoint("+c0 +") "+style+")" );
						
						break;						

					case "mpp":
						
						var p0 = ul.find("select[name='point.mpp.a']").val();
						var p1 = ul.find("select[name='point.mpp.b']").val();
						rows.push( id+"=point((("+ p0+"+"+p1+")/2) "+style+")" );
						
						break;						
						
					case "icc":
						
						var c0 = ul.find("select[name='point.icc.a']").val();
						var c1 = ul.find("select[name='point.icc.b']").val();
						
						var solutionindex = ul.find("select[name='point.icc.index']").val();						
						rows.push( id+"=point( index(intercept("+c0+","+c1+"),"+solutionindex+") "+style+")" ); 
					...
					case "ill":
						...
						var l0 = ul.find("select[name='point.ill.a']").val();
						var l1 = ul.find("select[name='point.ill.b']").val();
												
						rows.push( id+"=point( intercept("+l0+","+l1+") "+style+")" );
						
					break;						
						
					case "icl":
						
						var c0 = ul.find("select[name='point.icl.c']").val();
						var l0 = ul.find("select[name='point.icl.l']").val();
												 
						var solutionindex = ul.find("select[name='point.icl.index']").val();						
						rows.push( id+"=point( index(intercept("+c0+","+l0+"),"+solutionindex+") "+style+")" );
						
					break;						
					
					case "blp":
						
						var l0 = ul.find("select[name='point.blp.l']").val();
						var p0 = ul.find("select[name='point.blp.p']").val();
						rows.push( id+"=point(vertical("+l0+", "+p0+") "+style+")" );						
					...
					case "vec":
						...
						var vector = ul.find("select[name='point.vec']").val();
						
						rows.push( id+"=point(("+clearinput(vector)+")"+style+")" ); 
					break;
	...
	var parameters = {"in": rows, trig:"rad", sketch:1, p:displayoptions.precision, s:displayoptions.scientific };
	
	if (self.id!=null) {
		parameters["id"]=self.id;
		
		parameters["json"]=self.savetemp();
	}
	...
	$.ajax({
         ...
         success: function (json) {
        	 ...
        	 var results = json.results;
	        	 ...
	        	 for (var i=0;i<results.length && i<lis.length;i++)
	    		 {
	        		 var r = results[i];
	        		 
	        		 var li = lis[i];
	        		  
	        		 var outputinput = li.find("input.result");
	...
	var params = $.param( { r:rows, trig:(self.trigodeg.is(':checked') ?"deg":"rad"), b:self.axisbounds, w:graph.width(), h:graph.height(), c:Math.random() } );
	
	 
	var bgimg = $("<img />");
	var url = "sketch?"+params;
	bgimg.load(function(){
		
	  graph.css({opacity:1,backgroundPosition:"0px 0px", backgroundImage:("url(\""+url+"\")") } );
	//self.loading.hide();
	});
	bgimg.attr("src", url);
...
function Constants() {
	this.loadurl = caidat.host+"js/modal/constants.jsp";
	this.calc = null;
	this.modal = null;
}
...
Update.prototype.update = function(){
	...
	try {
		...
		if (question.length>0) {
			var voting = question.find(".voting");
			if (voting.length>0) {
				var seouri = voting.attr("name");
	        ...
	        success: function (json) {
	        		...
	        		if (json.userstyping) {
	        			...
	        			if (userstyping.length>0) {
	        				...
	        				var memberlink = $("a.username:first").attr("href");
	        				if (memberlink.indexOf("/")>=0)
	        					memberlink = memberlink.split("/")[1];
	        				
	        				
	        				
	        				userstyping.find("strong").text( ""+json.userstyping.length );
	        				var ul = userstyping.find("ul");
	        				ul.empty();
	        				for (var i=0;i<json.userstyping.length;i++) {
	        					var user = json.userstyping[i];
	        					var a = $("<li><a title=\""+user.username+"\" href=\"/"+memberlink+"/"+user.username+"/\"><img class=\"w32 avatarimg\" alt=\"avatar\" src=\"/img/avatars-32/"+user.username+".png?v="+user.v+"\"> <strong>"+user.username+"</strong></a></li>");
	        					a.appendTo(ul);
	        				}
	        				
	        				
	        				if (json.userstyping.length>1) 
	        					userstyping.addClass("plural");
	        				else
	        					userstyping.removeClass("plural");
	        				
	        				if (json.userstyping.length>0) 
	        					userstyping.show();
	        				else
	        					userstyping.hide();
	        			}
	        		...
	        		if (json.msgs) {
	        			...
	        			var msgbtn = $("#msgbtn");
	        			if (msgbtn.length>0) {
		        			msgbtn.find(".num").text( json.msgs );
		        			
		        			if (json.msgs>0)
		        				msgbtn.parent().removeClass("hidden");
		        			else
		        				msgbtn.parent().addClass("hidden");
	        			}
...
function initCalc() {
	...
	try{
		graph = new Graph();
		graph.trigodeg = calc.trigodeg;
		graph.trigorad = calc.trigorad;
		graph.graphinputs = calc.graphinputs;
		graph.axisbounds = calc.axisbounds;
		graph.graph = calc.graph;
		graph.init();
		//graph.show();
	} catch(e) {
		sendErrorReport("graph.init:", e);
	}
