...
if(window.location.protocol + "//" + location.host=='http://www.tienganh123.com'){
	style_box_nonvip='<style>#box_audio_nonvip{background-image: url(/file/common/libAudio/images/vip_bg.png);color: #fff;display: none;position: fixed;bottom: 3px;z-index: 9999;border-radius: 8px;background-color: #ee7600;background-repeat: no-repeat;width: 30%;left: 30%;height: 50px;padding:0px;}.box_vip_close {background: url(/file/common/libAudio/images/close.png) no-repeat;height: 22px;width: 22px;position: absolute;top: 10px;right: 10px;cursor: pointer;}</style>'
}
var str_nonvip=style_box_nonvip+'<div id="box_audio_nonvip" class="box_alert_vip">\
<div class="box_vip_text" style="margin:7px 15px 0px 15px">Bạn phải là <a href="/huong-dan/214-quyen-loi-thanh-vien-vip-cua-tienganh123.html" target="_blank" title="quyền lợi thành viên VIP" style="color:#F2FF32; text-decoration:underline; font-weight:bold">thành viên VIP</a> của TiếngAnh123.Com <br>mới nghe được phần này.</div><div class="box_vip_close"></div></div>';
var str_nonkid=style_box_nonvip+'<div id="box_audio_nonvip" class="box_alert_vip">\
<div class="box_vip_text" style="margin:7px 15px 0px 15px">Bạn phải là <a href="/huong-dan/2138-chuong-trinh-hoc-tieng-anh-tre-em-online-goldenkids.html" target="_blank" title="quyền lợi thành viên GoldenKid" style="color:#F2FF32; text-decoration:underline; font-weight:bold">thành viên GoldenKid</a> của TiếngAnh123.Com mới nghe được phần này.</div><div class="box_vip_close"></div></div>';
var lesson_vip=0,str_nonmember=0;

function PlayingAudioHide(element){
		if(element.hasClass('uba_hideAudio')){
			element.children('.stt_par_hideaudio').children(0).attr('src','http://www.tienganh123.com/file/m_tienganh123/audio/css/img_uban/speaker-playing.gif');
	...
	function stopAudioHide(element){
		if(element.hasClass('uba_hideAudio')){
			element.children('.stt_par_hideaudio').children(0).attr('src','http://www.tienganh123.com/file/m_tienganh123/audio/css/img_uban/speaker-stop.gif');
		}
	}
(function($){
	var defaults = {
		...
		methods={
			...
			if(currentTrack.indexOf('vip_only_audio')>0 || currentTrack.indexOf('goldenkids_only_audio')>0){
					 if($('#box_audio_nonvip').length<1){
						  $('body').append(str_nonmember);
					 }else{
						 $('#box_audio_nonvip').show();
					 }
					...
					$('#box_audio_nonvip').stop().animate({height: 'toggle'});
				}
			},
...
function PlayingAudioHide2(element){
	if(element.hasClass('uba_ctrl_game_hide')){
		element.children('span').children(0).attr('src','http://www.tienganh123.com/file/common/libAudio/images/audio_playing_bt.gif');
		if(prev_element != null){
			if(element.attr('media-url') != prev_element.attr('media-url')){
...
function stopAudioHide2(element){
	if(element.hasClass('uba_ctrl_game_hide')){
		element.children('span').children(0).attr('src','http://www.tienganh123.com/file/common/libAudio/images/audio_stop_bt.gif');
	}
}
