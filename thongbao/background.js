...
$(function(){
	$.material.init();
	if(typeof localStorage.bs_background_lastSeen != "undefined"){
		$("#"+localStorage.bs_background_lastSeen+"-tab").trigger("click")
	}
	...
	$("#mngoiy").click(function(){
		chrome.tabs.create({url: "http://www.bsquochoai.ga/2015/08/huong-dan-cai-at-i-hate-coc-coc-tren_18.html"});
	})
		 ...
		 if(tabid == "chromevpn"){
			chrome.storage.local.get("bs_chromevpn", function(re){
				if(!re.bs_chromevpn.isActivated){
					localStorage.bs_background_lastSeen = "caidatgiongnoi"
				} else {
					localStorage.bs_background_lastSeen = "chromevpn"
				}
			 })
		 } else {
		 	localStorage.bs_background_lastSeen = tabid
		 }
		})
...
if(localStorage.cddoc =="cddocgt"){
 ...
 $('[data-toggle="tooltip"]').tooltip()
  ...
  //Cài đặt các plugin khác
  var plugin = "/bsquochoai_plugins"
  
  $('a[data-toggle="tab"][data-plugin]').on('shown.bs.tab', function (e) {
	bs_modal = $($(e.target).attr("href"))
		if(bs_modal.data("ok") != "1"){
			bs_modal.html('<iframe src="'+plugin+$(e.target).data("open")+'"  style="width:100%; height:'+$("#myTabContent").height()+'px; border:none"></iframe>')
			bs_modal.data("ok", "1")
		}
	})
